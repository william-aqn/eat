...
__name(seal, "seal");
async function unseal(sealed, keyB64) {
  const key = await importKey(keyB64);
...
var SCOPES = "openid email https://www.googleapis.com/auth/drive.appdata";
...
function emailFromIdToken(idToken) {
  try {
    ...
    return typeof payload.email === "string" ? payload.email : void 0;
  } catch {
    return void 0;
  }
}
__name(emailFromIdToken, "emailFromIdToken");
...
app.get("/auth/callback", async (c) => {
  ...
  const session = {
    ...
    email: body.id_token ? emailFromIdToken(body.id_token) : void 0
  };
...
app.post("/auth/token", async (c) => {
  ...
  let session;
  try {
    session = await unseal(sealed, c.env.COOKIE_KEY);
  } catch {
...
app.post("/auth/logout", async (c) => {
  ...
  if (sealed) {
    try {
      const session = await unseal(sealed, c.env.COOKIE_KEY);
...
// .wrangler/tmp/bundle-yPLkMu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yPLkMu/middleware-loader.entry.ts
