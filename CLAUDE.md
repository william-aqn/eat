...
- Запрещённые продукты: store `forbidden` в IDB (версия БД 2), модуль `forbidden.ts`. id = нормализованное название (`normalizeFood`: trim+lowercase+схлопнутые пробелы) — один продукт с разных устройств сливается сам. Удаление = tombstone, текст НЕ затирается (id и так содержит название). В `entries.json` — необязательное поле `forbidden`; merge/planImport в `merge.ts` — дженерики, общие с записями. Подсветка позиций — `getForbiddenKeys()` (Set нормализованных названий).
