...
- Статусы auth: `signedOut` (локальный режим) / `signedIn` / `needsReauth` (нужен клик). В `sync.ts` при пустом токене: `signedOut` → `localOnly`, иначе → `needsAuth` («нажмите «Синхронизировать»»). Автотриггеры при протухшем токене молча ничего не делают — это и есть офлайн-режим по умолчанию.
